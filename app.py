...
class API:
    ...
    def get_config(self):
        return {
            ...
            "TRACKER_TR4KER":     os.getenv("TRACKER_TR4KER", ""),
    ...
    def _torrent_sb(self, params):
        try:
            ...
            announces = {
                ...
                "TR4KER": os.getenv("TRACKER_TR4KER", ""),
    ...
    def _create_torrent_via_ssh(self, base, remote_path, announce, private, tk_name, auto_start=True):
        ...
        # ── 4. SFTP download du .torrent (sans supprimer — gardé pour rtxmlrpc) ──
        sftp = client.open_sftp()
        try:
            try:
            ...
            sftp.close()
        # Ne pas fermer client ici — on en a besoin pour rtxmlrpc (step 5)
        ...
        # ── 5. Charger dans rtorrent ──────────────────────────────────────────
        # Stratégie : rtxmlrpc via SSH (SCGI direct, aucun proxy PHP)
        # → les slashes du chemin ne sont jamais touchés.
        # Fallback : HTTP XML-RPC via ruTorrent /RPC2 si rtxmlrpc absent.
        import hashlib as _hl, time as _t

        # Chemin parent via opération string pure (Path() sur Windows convertit en \)
        parent_dir = remote_path.rstrip("/").rsplit("/", 1)[0] or "/"

        # ── Extraire l'info hash (SHA1 du dict info bencoded) ─────────────────
        def _info_hash(tb):
            marker = b"4:info"
            idx = tb.find(marker)
            if idx < 0:
        ...
        if rxtool and info_hash:
            try:
                # 5a. Charger sans démarrer (load.normal = load sans start)
                o, e = _exec(f"{rxtool} load.normal '' '{tmp_remote}'", timeout=30)
                self._log(f"  [SSH] rtxmlrpc load.normal → {(o or e or 'ok')[:200]}")

                _t.sleep(1)  # rtorrent enregistre le torrent

                # 5b. Définir le répertoire — SCGI direct, aucune sanitisation
                o, e = _exec(
        ...
        if not loaded_ok and rt_url:
            ...
            # Note : le proxy PHP peut modifier les slashes du chemin —
            # si le répertoire est toujours mauvais, vérifier que rtxmlrpc est dispo.
            self._log("  [ruT] Fallback HTTP XML-RPC…", "warn")
            try:
    ...
    # ─── CUSTOM MUXER ─────────────────────────────────────────────────────────
    def custom_scan(self) -> dict:
        """Liste les fichiers dans BASE_DIR/CUSTOM/ et vérifie mkvmerge."""
        import shutil as _sh
        custom_dir = BASE_DIR / "CUSTOM"
        custom_dir.mkdir(exist_ok=True)
        video_exts = {".mkv", ".mp4", ".avi", ".m4v", ".mov", ".ts", ".m2ts", ".mts"}
        sub_exts   = {".srt", ".ass", ".ssa", ".sub", ".vtt"}
        items = []
        # Fichiers système à ignorer
        _IGNORE = {".ds_store", "thumbs.db", "desktop.ini", ".localized"}
        for f in sorted(custom_dir.iterdir()):
            if not f.is_file():
                continue
            if f.name.lower() in _IGNORE or f.name.startswith("._"):
                continue
            ext = f.suffix.lower()
            ftype = "video" if ext in video_exts else "subtitle" if ext in sub_exts else "other"
            items.append({"name": f.name, "path": str(f), "ext": ext.lstrip("."), "type": ftype,
                          "size": f.stat().st_size})
        mkv = _sh.which("mkvmerge") or ""
        if not mkv:
            for c in [r"C:\Program Files\MKVToolNix\mkvmerge.exe",
                      "/opt/homebrew/bin/mkvmerge", "/usr/local/bin/mkvmerge", "/usr/bin/mkvmerge"]:
                if os.path.isfile(c):
                    mkv = c
                    break
        return {"files": items, "mkvmerge": mkv}

    def custom_identify(self, path: str) -> dict:
        """Identifie les pistes d'un fichier via mkvmerge --identify JSON."""
        import shutil as _sh, subprocess as _sp, json as _json
        ext = os.path.splitext(path)[1].lower()
        # Fichier sous-titre mono-piste : détecter langue + type depuis le nom
        if ext in (".srt", ".ass", ".ssa", ".sub", ".vtt"):
            n = os.path.basename(path).lower()
            # ── Langue ──────────────────────────────────────────────────────
            lang = "und"
            _lang_rules = [
                ("fre", ["vostfr","french","_fr.","_fr_",".fr.","fra.","fre.","-fr.","-fr-","fr_full","fr_forced"]),
                ("eng", ["vost","english","_en.","_en_",".en.","eng.","-en.","-en-","en_full","en_forced"]),
                ("deu", ["german","deutsch","_de.","_de_",".de.","ger.","deu.","-de."]),
                ("spa", ["spanish","espanol","_es.","_es_",".es.","spa.","-es."]),
                ("ita", ["italian","italiano","_it.","_it_",".it.","ita.","-it."]),
                ("por", ["portuguese","portugues","_pt.","_pt_",".pt.","por.","-pt."]),
                ("jpn", ["japanese","japonais","_jp.","_jp_",".jp.","jpn.","-jp."]),
                ("chi", ["chinese","chinois","_zh.","_zh_",".zh.","chi.","-zh."]),
                ("kor", ["korean","coreen","_ko.","_ko_",".ko.","kor.","-ko."]),
                ("rus", ["russian","russe","_ru.","_ru_",".ru.","rus.","-ru."]),
                ("ara", ["arabic","arabe","_ar.","_ar_",".ar.","ara.","-ar."]),
                ("pol", ["polish","polonais","_pl.","_pl_",".pl.","pol.","-pl."]),
                ("dut", ["dutch","neerlandais","_nl.","_nl_",".nl.","nld.","-nl."]),
            ]
            for code, patterns in _lang_rules:
            ...
            forced = False
            sub_name = ""
            _short = {"fre":"FR","eng":"EN","deu":"DE","spa":"ES","ita":"IT","por":"PT",
                       "jpn":"JP","chi":"ZH","kor":"KO","rus":"RU","ara":"AR","pol":"PL","dut":"NL"}
            short = _short.get(lang, lang[:2].upper() if lang != "und" else "")
            if any(x in n for x in ["forced","force","forcé"]):
                forced = True
                sub_name = f"{short} FORCED" if short else ""
            elif any(x in n for x in ["sdh","hearing"]):
                sub_name = f"{short} SDH" if short else ""
            elif short:
                sub_name = f"{short} FULL"
            return {"ok": True, "tracks": [
                {"id": 0, "type": "subtitles", "codec": ext.lstrip(".").upper(),
                 "language": lang, "name": sub_name, "default": False, "forced": forced}
            ]}
        mkv = _sh.which("mkvmerge") or ""
        if not mkv:
            for c in [r"C:\Program Files\MKVToolNix\mkvmerge.exe",
                ...
                if os.path.isfile(c):
                    ...
                    break
        if not mkv:
            return {"ok": False, "error": "mkvmerge introuvable"}
        try:
            r = _sp.run([mkv, "--identification-format", "json", "--identify", path],
                        capture_output=True, text=True, timeout=20,
                        encoding="utf-8", errors="replace")
            data = _json.loads(r.stdout)

            # ── helpers de classification (miroir de mkvtoolnix_remux.py) ──────
            def _cls_sub(props):
                """Classifie une piste sous-titre : FORCED / SDH / COMMENTARY / FULL / NORMAL."""
                nm = (props.get("track_name") or "").upper()
                if bool(props.get("forced_track")) or "FORCED" in nm:
                    return "FORCED"
                if "COMMENT" in nm:
                    return "COMMENTARY"
                if "SDH" in nm or "HEARING" in nm:
                    return "SDH"
                if "FULL" in nm or "COMPLET" in nm:
                    return "FULL"
                return "NORMAL"

            def _det_vf(props):
                """Détecte la variante VF depuis le nom de piste source."""
                nm = (props.get("track_name") or "").upper()
                if "VFF" in nm or "FRANCE" in nm:      return "VFF"
                if "VFQ" in nm or "QUEBEC" in nm or "QUÉBEC" in nm: return "VFQ"
                if "VFI" in nm or "INTERNATIONAL" in nm: return "VFi"
                if "VOF" in nm or "VERSION ORIGINALE" in nm: return "VOF"
                return ""

            # ── construction des pistes ──────────────────────────────────────
            tracks = []
            sub_types_by_lang = {}          # lang → [sub_types déjà assignés]
            for t in data.get("tracks", []):
                p = t.get("properties", {})
                lang  = p.get("language", "und")
                ttype = t.get("type", "")

                # Classification sous-titre
                sub_type = ""
                if ttype == "subtitles":
                    st = _cls_sub(p)
                    if st == "NORMAL":
                        # Heuristique de position : même logique que le remux tool
                        existing = sub_types_by_lang.get(lang, [])
                        if not existing:
                            st = "FORCED"   # première piste de cette langue → FORCED
                        elif "FULL" not in existing:
                            st = "FULL"
                        else:
                            st = "FULL"     # fallback
                    sub_types_by_lang.setdefault(lang, []).append(st)
                    sub_type = st

                # Détection variante VF pour piste audio française
                vf_type = ""
                if ttype == "audio" and lang in ("fre", "fra", "que"):
                    vf_type = _det_vf(p)
                ...
                tracks.append({
                    ...
                    "default":     bool(p.get("default_track", False)),
                    "forced":      bool(p.get("forced_track", False)),
                    "channels":    p.get("audio_channels", None),
                    "bitrate_kbps": 0,
                    "sub_type":    sub_type,    # FORCED / SDH / FULL / COMMENTARY
                    "vf_type":     vf_type,     # VFF / VFQ / VFi / VOF / ""
                })
            # Enrichir les pistes audio avec le bitrate via pymediainfo
            try:
                from pymediainfo import MediaInfo as _MI
                mi = _MI.parse(path)
                br_by_order = {}
                ai = 0
                for mt in mi.tracks:
                    if mt.track_type == "Audio":
                        br = getattr(mt, "bit_rate", None)
                        if br:
                            try:
                                br_by_order[ai] = int(br) // 1000
                            except Exception:
                                pass
                        ai += 1
                audio_order = 0
                for tr in tracks:
                    if tr["type"] == "audio":
                        tr["bitrate_kbps"] = br_by_order.get(audio_order, 0)
                        audio_order += 1
            except Exception:
                pass  # pymediainfo absent ou erreur → pas de bitrate, pas grave
            return {"ok": True, "tracks": tracks}
        except Exception as e:
            return {"ok": False, "error": str(e)}

    def custom_nfo_preview(self, config: dict) -> dict:
        """
        Génère un aperçu texte du NFO CUSTOM à partir de la config UI.
        Utilise MediaInfo pour les stats vidéo ; les noms de pistes viennent de la config.
        """
        import unicodedata, datetime as _dt
        try:
            from pymediainfo import MediaInfo as _MI
        except ImportError:
            return {"ok": False, "error": "pymediainfo non disponible"}

        # ── trouver le fichier vidéo coché ───────────────────────────────────
        video_path = None
        video_name_track = ""
        for f in config.get("files", []):
            ext = f.get("ext", "").lower()
            if ext not in ("mkv","mp4","avi","m4v","mov","ts","m2ts","mts"):
                continue
            for t in f.get("tracks", []):
                if t.get("type") == "video" and t.get("include", True):
                    video_path = f["path"]
                    video_name_track = t.get("name", "")
                    break
            if video_path:
                break
        if not video_path:
            return {"ok": False, "error": "Aucun fichier vidéo coché"}

        # ── MediaInfo vidéo ──────────────────────────────────────────────────
        try:
            mi = _MI.parse(video_path)
        except Exception as e:
            return {"ok": False, "error": str(e)}

        gen = next((t for t in mi.tracks if t.track_type == "General"), None)
        vid = next((t for t in mi.tracks if t.track_type == "Video"), None)

        # Codec
        _codec_map = {"HEVC":"HEVC","AVC":"AVC","AV1":"AV1","VP9":"VP9","VC-1":"VC-1","MPEG-4 Visual":"MPEG-4"}
        vcodec = _codec_map.get(vid.format or "", vid.format or "N/A") if vid else "N/A"
        # Bitrate vidéo
        vbr_raw = getattr(vid, "bit_rate", None) or 0
        vbr = f"{int(vbr_raw)//1000:,} kb/s".replace(",", " ") if vbr_raw else "N/A"
        # Résolution
        w = getattr(vid, "width", 0) or 0
        h = getattr(vid, "height", 0) or 0
        if h >= 2160: res = "2160p UHD"
        elif h >= 1080: res = "1080p"
        elif h >= 720:  res = "720p"
        else:           res = f"{h}p" if h else "N/A"
        # Aspect ratio
        ar = getattr(vid, "display_aspect_ratio", "") or ""
        if ar and "." in ar:
            try:
                from fractions import Fraction
                f_ar = Fraction(ar).limit_denominator(100)
                ar = f"{f_ar.numerator}:{f_ar.denominator}"
            except Exception:
                pass
        # FPS
        fps = getattr(vid, "frame_rate", "") or "N/A"
        # Durée
        dur_ms = getattr(vid, "duration", None) or getattr(gen, "duration", None) or 0
        if dur_ms:
            dur_s = int(float(dur_ms)) // 1000
            hh = dur_s // 3600; mm = (dur_s % 3600) // 60
            duration = f"{hh}h {mm:02d}min"
        else:
            duration = "N/A"
        # HDR
        hdr_raw = " ".join(filter(None, [
            str(getattr(vid, "hdr_format", "") or ""),
            str(getattr(vid, "other_hdr_format", "") or ""),
        ]))
        hdr_parts = []
        if "Dolby Vision" in hdr_raw: hdr_parts.append("Dolby Vision")
        if "HDR10+" in hdr_raw:       hdr_parts.append("HDR10+")
        elif "HDR10" in hdr_raw or ("HDR" in hdr_raw and "Dolby" not in hdr_raw):
            hdr_parts.append("HDR10")
        hdr_str = " / ".join(hdr_parts)
        # Taille fichier
        size_bytes = os.path.getsize(video_path) if os.path.isfile(video_path) else 0
        size_gib = f"{size_bytes / (1024**3):.2f} GiB"

        # ── Pistes audio depuis la config ────────────────────────────────────
        _LANG_MAP = {
            "fre":"FRENCH","fra":"FRENCH","que":"FRENCH",
            "eng":"ENGLISH","deu":"GERMAN","ger":"GERMAN",
            "spa":"SPANISH","esl":"SPANISH","ita":"ITALIAN",
            "por":"PORTUGUESE","jpn":"JAPANESE","chi":"CHINESE","zho":"CHINESE",
            "kor":"KOREAN","rus":"RUSSIAN","nld":"DUTCH","dut":"DUTCH",
            "pol":"POLISH","swe":"SWEDISH","nor":"NORWEGIAN","dan":"DANISH",
        }
        _CHAN_MAP = {1:"1.0",2:"2.0",6:"5.1",7:"6.1",8:"7.1"}
        audio_tracks = []
        sub_tracks   = []
        for f in config.get("files", []):
            for t in f.get("tracks", []):
                if not t.get("include", True):
                    continue
                ttype = t.get("type","")
                lang_code = t.get("language","und")
                lang_label = _LANG_MAP.get(lang_code, lang_code.upper())
                name = t.get("name","")

                if ttype == "audio":
                    ch = int(t.get("channels") or 0)
                    channels = _CHAN_MAP.get(ch, f"{ch}.0" if ch else "N/A")
                    br = t.get("bitrate_kbps", 0) or 0
                    br_str = f"{br:,} kb/s".replace(",", " ") if br else "N/A"
                    # Codec normalisé depuis le nom de piste
                    _cn = name.upper()
                    if "TRUEHD" in _cn and "ATMOS" in _cn: acodec = "TrueHD Atmos"
                    elif "TRUEHD" in _cn:                   acodec = "TrueHD"
                    elif "DTS-HD MA" in _cn:                acodec = "DTS-HD MA"
                    elif "DTS-HD" in _cn:                   acodec = "DTS-HD"
                    elif "DTS" in _cn:                      acodec = "DTS"
                    elif "EAC3" in _cn or "E-AC" in _cn:   acodec = "EAC3 (Atmos)" if "ATMOS" in _cn else "EAC3"
                    elif "AC3" in _cn:                      acodec = "AC3"
                    elif "AAC" in _cn:                      acodec = "AAC"
                    elif "FLAC" in _cn:                     acodec = "FLAC"
                    elif "OPUS" in _cn:                     acodec = "Opus"
                    else:                                    acodec = t.get("codec","N/A")
                    # Langue affichée : FRENCH (VFF) si VF
                    name_up = name.upper()
                    if lang_label == "FRENCH":
                        for vf in ("VFF","VFQ","VFi","VFI","VOF","VF2"):
                            if name_up.startswith(vf):
                                lang_label = f"FRENCH ({vf})"
                                break
                    audio_tracks.append({
                        "language": lang_label, "channels": channels,
                        "codec": acodec, "bitrate": br_str, "name": name,
                    })

                elif ttype in ("subtitles","subtitle"):
                    name_up = name.upper()
                    if   "FORCED"      in name_up: stype = "FORCED"
                    elif "SDH"         in name_up: stype = "SDH"
                    elif "COMMENTARY"  in name_up: stype = "COMMENTARY"
                    else:                           stype = "FULL"
                    codec = (t.get("codec","") or "SRT").upper()
                    sub_tracks.append({
                        "language": lang_label, "type": stype,
                        "format": codec, "name": name,
                    })

        # ── Construction du NFO ──────────────────────────────────────────────
        W = 79  # largeur intérieure entre les deux █

        def _vlen(s):
            tot = 0
            for c in s:
                if unicodedata.category(c) in ('Mn','Mc','Me'): continue
                tot += 2 if unicodedata.east_asian_width(c) in ('W','F') else 1
            return tot
        ...
        def L(inner=""):   return f"█{_vljust(' ' + inner, W)}█"
        def C(inner=""):   return f"█{_vcenter(inner)}█"
        def TOP():         return "█▀" + "▀" * 77 + "▀█"
        def BOT():         return "█▄" + "▄" * 77 + "▄█"
        def HDR(title):    return f"█ ████▓▓▓▒▒▒░░░{_vcenter(title, 45)}░░░▒▒▒▓▓▓████ █"

        def field(label, value, lw=20, rw=38, label2="", value2=""):
            """Ligne à 1 ou 2 champs."""
            left = f"■ {label}: {_vljust(value, lw)}"
            if label2:
                right = f"■ {label2}: {_vljust(value2, rw)}"
                inner = f" {left}  {right}"
                return f"█{_vljust(inner, W)}█"
            return L(f"■ {label}: {value}")

        def field2(l1, v1, l2, v2, lw=18, rw=18):
            inner = f" ■ {l1}: {_vljust(v1, lw)}  ■ {l2}: {_vljust(v2, rw)}"
            return f"█{_vljust(inner, W)}█"

        release_name = config.get("output", "").replace(".mkv", "")
...
"█                                                                               █",
        ...
        # ── Release infos ─────────────────────────────────────────────────────
        lines += [TOP(), HDR("RELEASE iNFOS"), BOT(), L()]
        lines.append(C(release_name))
        lines.append(L())
        lines.append(field2("RELEASE SiZE", size_gib, "RELEASE DATE", today, 18, 18))
        lines.append(L(f"■ SOURCE..........: CUSTOM.REMUX"))
        lines.append(L())
        # ── Video ─────────────────────────────────────────────────────────────
        lines += [TOP(), HDR("ViDEO"), BOT(), L()]
        if video_name_track:
            lines.append(L(f"■ TiTLE...........: {video_name_track}"))
        lines.append(field2("CODEC", vcodec, "BiTRATE", vbr, 18, 18))
        lines.append(field2("RESOLUTiON", res, "ASPECT RATiO", ar, 18, 18))
        lines.append(field2("FRAMERATE", fps + " FPS", "DURATiON", duration, 18, 18))
        if hdr_str:
            lines.append(L(f"■ HDR.............: {hdr_str}"))
        lines.append(L())
        # ── Audio ─────────────────────────────────────────────────────────────
        for idx, a in enumerate(audio_tracks, 1):
            lines += [TOP(), HDR(f"AUDiO #{idx}"), BOT(), L()]
            lines.append(field2("LANGUAGE", a["language"], "CHANNELS", a["channels"], 28, 8))
            lines.append(field2("CODEC", a["codec"], "BiTRATE", a["bitrate"], 28, 18))
            lines.append(L())
        # ── Subtitles ─────────────────────────────────────────────────────────
        for idx, s in enumerate(sub_tracks, 1):
            lines += [TOP(), HDR(f"SUBTiTLES #{idx}"), BOT(), L()]
            lines.append(field2("LANGUAGE", s["language"], "TYPE", s["type"], 28, 18))
            lines.append(L(f"■ FORMAT..........: {s['format']}"))
            lines.append(L())
        # ── Pied ──────────────────────────────────────────────────────────────
        lines += [
TOP(),
...
"▀█▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄█▀",
    ...
    def custom_mux(self, config: dict) -> dict:
        """
        Muxe les fichiers configurés → FILMS/
        config = {
          "output": "Film.2025.FRENCH.mkv",
          "files": [
            { "path": "/abs/…/file.mkv", "ext": "mkv",
              "tracks": [
                {"id": 0, "type": "video",     "include": True, "language": "und", "name": "REBiRTH"},
                {"id": 1, "type": "audio",     "include": True, "language": "fre", "name": "VFF"},
                {"id": 2, "type": "subtitles", "include": False,"language": "fre", "name": ""},
              ]
            },
            { "path": "/abs/…/sub.srt", "ext": "srt",
              "tracks": [{"id": 0, "type": "subtitles", "include": True, "language": "fre",
                          "name": "Français FULL", "forced": False}]
            }
          ]
        }
        """
        ...
        output = str(films_dir / config["output"])
        mkv = _sh.which("mkvmerge") or ""
        if not mkv:
            for c in [r"C:\Program Files\MKVToolNix\mkvmerge.exe",
                      "/opt/homebrew/bin/mkvmerge", "/usr/local/bin/mkvmerge", "/usr/bin/mkvmerge"]:
                if os.path.isfile(c):
                    ...
                    break
        if not mkv:
            return {"ok": False, "error": "mkvmerge introuvable — installe MKVToolNix"}
        sub_exts = {"srt", "ass", "ssa", "sub", "vtt"}
        cmd = [mkv, "-o", output]
        for fconf in config.get("files", []):
            path   = fconf["path"]
            ext    = fconf.get("ext", "").lower()
            tracks = fconf.get("tracks", [])
            if ext in sub_exts:
                t = tracks[0] if tracks else {}
                if t.get("language"):
                    cmd += ["--language", f"0:{t['language']}"]
                if t.get("name"):
                    cmd += ["--track-name", f"0:{t['name']}"]
                if t.get("forced"):
                    cmd += ["--forced-track", "0:1"]
                cmd += ["--sub-charset", "0:UTF-8", path]
            else:
                included = [t for t in tracks if t.get("include")]
                if not included:
                    continue
                all_v  = [t for t in tracks if t.get("type") == "video"]
                all_a  = [t for t in tracks if t.get("type") == "audio"]
                all_s  = [t for t in tracks if t.get("type") == "subtitles"]
                inc_v  = [t for t in included if t.get("type") == "video"]
                inc_a  = [t for t in included if t.get("type") == "audio"]
                inc_s  = [t for t in included if t.get("type") == "subtitles"]
                if not inc_v:
                    cmd.append("--no-video")
                elif len(inc_v) < len(all_v):
                    cmd += ["--video-tracks", ",".join(str(t["id"]) for t in inc_v)]
                if not inc_a:
                    cmd.append("--no-audio")
                elif len(inc_a) < len(all_a):
                    cmd += ["--audio-tracks", ",".join(str(t["id"]) for t in inc_a)]
                if not inc_s:
                    cmd.append("--no-subtitles")
                elif len(inc_s) < len(all_s):
                    cmd += ["--subtitle-tracks", ",".join(str(t["id"]) for t in inc_s)]
                for t in included:
                    tid = str(t["id"])
                    if t.get("language"):
                        cmd += ["--language",    f"{tid}:{t['language']}"]
                    if t.get("name"):
                        cmd += ["--track-name",  f"{tid}:{t['name']}"]
                    if t.get("forced"):
                        cmd += ["--forced-track", f"{tid}:1"]
                    if t.get("default"):
                        cmd += ["--default-track", f"{tid}:1"]
                cmd.append(path)
        try:
            ...
            return {"ok": False, "error": "Timeout 10 min dépassé"}
        except Exception as e:
            return {"ok": False, "error": str(e)}
